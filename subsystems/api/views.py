...
def user_deals(request):
    ...
    deals_out = Deal.objects.filter(user_from=request.user).exclude(venue__owner=None)
    deals_in = Deal.objects.filter(user_to=request.user).exclude(venue__owner=None)
